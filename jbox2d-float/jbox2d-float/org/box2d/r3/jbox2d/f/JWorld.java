...
public class JWorld implements Box2DWorld {
	...
	private org.jbox2d.f.dynamics.World gdx_world;

	// private Box2DWorldRenderer renderer;

	public JWorld(Box2DWorldSpecs specs) {
		Float2 g = specs.getGravity();
		Vector2 gravity = new Vector2((float) g.getX(), (float) g.getY());
		gdx_world = new org.jbox2d.f.dynamics.World(gravity);
	...
	public Box2DBody createBody(BodyDef boxBodyDef) {
		return new JBox2DBody(
				gdx_world.createBody(((JBodyDef) (boxBodyDef))
						.getGdxBodyDef()));
	}
	...
	public void step(double box2d_time_step_delta, int velocityIterations,
		...
		gdx_world.step((float) box2d_time_step_delta, velocityIterations,
				positionIterations);
	}
